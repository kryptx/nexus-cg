...
describe("Network Module", function()
    ...
    describe("Network:isValidPlacement()", function()
        ...
        it("should be valid adjacent to Reactor if card has matching Input (Tech In vs Reactor Bot-Right Output)", function()
            ...
            -- Needs Input on its Top edge. Top-Right port (2) is Tech Input.
            -- Reactor's corresponding Bottom-Right port (4) is Tech Output & is open.
        ...
        it("should be valid adjacent to Reactor if card has matching Input (Res In vs Reactor Right-Bot Output)", function()
            ...
            -- Needs Input on its Left edge. Left-Bottom port (6) is Res Input.
            -- Reactor's corresponding Right-Bottom port (8) is Res Output & is open.
            local cardResourceInputOnly_local = Card:new({ id = "NODE_R_IN", type = Card.Type.RESOURCE, title = "Res In",
                definedPorts = { [Card.Ports.LEFT_BOTTOM] = true } -- Left-Bottom(6) is Res Input
            })
        ...
        it("should be invalid adjacent to Reactor if card has NO open Input on connecting edge", function()
            ...
            -- Needs Input on its Top edge (Ports 1, 2). Card only has Cult Output (Port 1 open).
        ...
        it("should be valid adjacent to Node if matching Output->Input exists", function()
            ...
            local card_T_TechOut = Card:new({ id="TTO", type=Card.Type.TECHNOLOGY, title="TTO",
                                             definedPorts={ [Card.Ports.TOP_RIGHT]=true,
                                                         [Card.Ports.BOTTOM_RIGHT]=true }
                                           })
        ...
        it("should be invalid adjacent to Node if no matching Output->Input exists (type mismatch)", function()
            ...
            local card_T_TechOut = Card:new({ id="TTO", type=Card.Type.TECHNOLOGY, title="TTO", definedPorts={ [Card.Ports.BOTTOM_RIGHT]=true, [Card.Ports.TOP_RIGHT]=true }})
            card_T_TechOut.owner = player -- Set owner
            network:placeCard(card_T_TechOut, 0, 1)

            -- Test: Try placing Cult Input card below it at (0,2)
            local card_B_CultIn = Card:new({ id="BCI", type=Card.Type.CULTURE, title="BCI", definedPorts={ [Card.Ports.TOP_LEFT]=true }})
        ...
        it("should be invalid adjacent to Node if required Input port on new card is closed", function()
             -- Setup: Place card providing Tech Output downwards at (0,1)
            local card_T_TechOut = Card:new({ id="TTO", type=Card.Type.TECHNOLOGY, title="TTO", definedPorts={ [Card.Ports.BOTTOM_RIGHT]=true, [Card.Ports.TOP_RIGHT]=true }})
            card_T_TechOut.owner = player -- Set owner
            network:placeCard(card_T_TechOut, 0, 1)

            -- Test: Place card needing Tech Input below it at (0,2), but its Input port is closed
            local card_B_TechIn_Closed = Card:new({ id="BTI", type=Card.Type.TECHNOLOGY, title="BTI", definedPorts={}})
        ...
        it("should be invalid adjacent to Node if matching Output port on adjacent card is closed", function()
            -- Setup: Place card providing NO Tech Output downwards at (0,1)
            local card_T_NoTechOut = Card:new({ id="TNTO", type=Card.Type.TECHNOLOGY, title="TNTO", definedPorts={ [Card.Ports.TOP_RIGHT]=true }})
            ...
            local card_B_TechIn = Card:new({ id="BTI", type=Card.Type.TECHNOLOGY, title="BTI", definedPorts={ [Card.Ports.TOP_RIGHT]=true }})
            card_B_TechIn.owner = player -- Set owner
            local isValid, reason = network:isValidPlacement(card_B_TechIn, 0, 2)
            assert.is_false(isValid)
            assert.matches("No valid connection found", reason, nil, true)
    ...
    describe("Network:findPathToReactor()", function()
        ...
        before_each(function()
            ...
            card_T_Out_B_In = Card:new({ id="T_OB", type=Card.Type.TECHNOLOGY, title="T_OB",
                                        definedPorts={ [Card.Ports.TOP_RIGHT]=true, [Card.Ports.BOTTOM_RIGHT]=true } })
            ...
            card_T_Cult_Out = Card:new({ id="T_CO", type=Card.Type.CULTURE, title="T_CO",
                                        definedPorts={ [Card.Ports.TOP_RIGHT]=true, [Card.Ports.BOTTOM_LEFT]=true } })
        ...
        it("should return nil if no valid Input<-Output path exists", function()
            ...
            local card_B_Cult_In = Card:new({ id="BCI", type=Card.Type.CULTURE, title="BCI", definedPorts={ [Card.Ports.TOP_LEFT]=true }})
