...
-- ==========================================
-- Tests for global activation execution
-- ==========================================
describe("ActivationService:attemptActivationGlobal", function()
    local gameService, service, p1, p2, net1, net2, p1_reactor, p1_inter, p2_target

    -- Reuse mocks for cards/players/networks
    local function createMockCard2(id, title, x, y)
        local card = createMockCard(id, title, x, y)
        return card
    end
    local createMockPlayer2 = createMockPlayer
    local createMockNetwork2 = createMockNetwork

    before_each(function()
        gameService = { players = {}, activeConvergenceLinks = {}, audioManager = { playSound = function() end } }
        service = ActivationService:new(gameService)
        -- Create players
        p1 = createMockPlayer2(1, { energy = 20 })
        p2 = createMockPlayer2(2, { energy = 10 })
        gameService.players = { p1, p2 }
        -- Create reactor and intermediate/target mocks
        p1_reactor = createMockCard2("P1_R", "P1 Reactor", 0, 0); p1_reactor.type = Card.Type.REACTOR
        p1_inter = createMockCard2("P1_I", "P1 Intermediate", 1, 0)
        p2_target = createMockCard2("P2_T", "P2 Target", 1, 1)
        -- Spy on intermediate activation
        p1_inter.activationEffect.activate = spy.new(function(gs, activatingPlayer, targetNetwork, targetNode) end)
        -- Assign networks
        net1 = createMockNetwork2({ p1_reactor, p1_inter }); p1.network = net1
        p1_reactor.owner, p1_reactor.network = p1, net1
        p1_inter.owner, p1_inter.network = p1, net1
    ...
    it("should activate subsequent nodes owned by activator, spending energy and passing correct targetNode", function()
        local initialEnergy = p1.resources.energy
        local success, msg = service:attemptActivationGlobal(1, 2, 1, 1)
        assert.is_true(success)
        -- Path cost minus 1 nodes = energy cost
        assert.are.equal(initialEnergy - 2, p1.resources.energy)
        -- Spy should have been called once
        assert.spy(p1_inter.activationEffect.activate).was.called(1)
        -- Check spy arguments: ActivationService instance, player, network, node
        assert.spy(p1_inter.activationEffect.activate).was.called_with(gameService, p1, net1, p1_inter)
    end)
